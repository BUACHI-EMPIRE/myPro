...
database_url = os.environ.get("DATABASE_URL", "sqlite:///sms_app.db")
...
class SMSCampaign(db.Model):
    ...
    status = db.Column(db.String(20), nullable=False, default=SMSStatus.PENDING.value)
    created_at = db.Column(db.DateTime, nullable=False, default=datetime.utcnow)
    completed_at = db.Column(db.DateTime, nullable=True)
    
    # Relationship to SMS records
    sms_records = db.relationship("SMSRecord", backref="campaign", lazy="dynamic", cascade="all, delete-orphan")
    invalid_numbers_rel = db.relationship("InvalidPhoneNumber", backref="campaign", lazy="dynamic", cascade="all, delete-orphan")
...
class SMSRecord(db.Model):
    ...
    def __repr__(self):
        return f'<SMSRecord {self.id}: {self.phone_number} - {self.status}>'
...
def statistics():
    ...
    overall_stats = {
        ...
        'success_rate': round((total_successful / total_messages * 100), 2) if total_messages > 0 else 0
    }
